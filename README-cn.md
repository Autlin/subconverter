...
[![Build Status](https://github.com/Autlin/subconverter/actions/workflows/build.yml/badge.svg)](https://github.com/Autlin/subconverter/actions)
[![GitHub tag (latest SemVer)](https://img.shields.io/github/tag/Autlin/subconverter.svg)](https://github.com/Autlin/subconverter/tags)
[![GitHub release](https://img.shields.io/github/release/Autlin/subconverter.svg)](https://github.com/Autlin/subconverter/releases)
[![GitHub license](https://img.shields.io/github/license/Autlin/subconverter.svg)](LICENSE)
...
4.  目标类型为 `auto` 时，会根据请求的 `User-Agent` 自动判断输出的目标类型，匹配规则可参见 [此处](src/handler/interfaces.cpp#L121) （该链接有可能因为代码修改而不能准确指向相应的代码）
...
在程序目录内的任意位置创建一个新的文档文件（推荐保存至 `profiles` 文件夹内，以使整洁目录及便于后续维护），如 `formyairport.ini`，并仿照 [示例文档](base/profiles/example_profile.ini) 根据配置好的参数填写进去即可。
...
注：本部分内容以本程序中的 [`pref.example.ini`](base/pref.example.ini) 或 [`pref.example.yml`](base/pref.example.yml) 或 [`pref.example.toml`](base/pref.example.toml) 为准，本文档可能由于更新不及时，内容不适用于新版本。
...
注：本部分内容以本程序中的 [`/config/example_external_config.ini`](base/config/example_external_config.ini) 或 [`/config/example_external_config.yml`](base/config/example_external_config.yml) 或 [`/config/example_external_config.toml`](base/config/example_external_config.toml) 为准，本文档可能由于更新不及时，内容不适用于新版本。
