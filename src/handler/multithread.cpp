...
std::shared_future<std::string> fetchFileAsync(const std::string &path, const std::string &proxy, int cache_ttl, bool find_local, bool async)
{
    ...
    const auto policy = async ? std::launch::async : std::launch::deferred;
    /*if(vfs::vfs_exist(path))
        ...
        retVal = std::async(policy, [path](){return fileGet(path, true);});
    else if(isLink(path))
        retVal = std::async(policy, [path, proxy, cache_ttl](){return webGet(path, proxy, cache_ttl);});
    else
