...
int uploadGist(std::string name, std::string path, std::string content, bool writeManageURL)
{
    ...
    if(!id.size())
    {
        ...
        retVal = webPost("https://api.github.com/gists", buildGistData(path, content), getSystemProxy(), {{"Authorization", "token " + token}}, &retData, true);
        if(retVal != 201)
        {
    ...
    else
    {
        ...
        retVal = webPatch("https://api.github.com/gists/" + id, buildGistData(path, content), getSystemProxy(), {{"Authorization", "token " + token}}, &retData, true);
        if(retVal != 200)
        {
